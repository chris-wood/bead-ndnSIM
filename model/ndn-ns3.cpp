...
template std::shared_ptr<const Bead>
Convert::FromPacket<Bead>(Ptr<Packet> packet);
...
template Ptr<Packet>
Convert::ToPacket<Bead>(const Bead& packet);
...
Convert::getPacketType(Ptr<const Packet> packet)
{
  ...
  if (type == ::ndn::tlv::Interest || type == ::ndn::tlv::Data || type == ::ndn::tlv::Bead) {
    return type;
  }
  else {
    throw ::ndn::tlv::Error("Unknown header");

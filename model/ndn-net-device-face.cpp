...
void
NetDeviceFace::sendBead(const Bead& bead)
{
  NS_LOG_FUNCTION(this << &bead);

  this->emitSignal(onSendBead, bead);

  Ptr<Packet> packet = Convert::ToPacket(bead);
  send(packet);
}
                                    ...
                                    NetDevice::PacketType packetType)
{
  ...
  try {
    ...
    else if (type == ::ndn::tlv::Bead) {
      shared_ptr<const Bead> d = Convert::FromPacket<Bead>(packet);
